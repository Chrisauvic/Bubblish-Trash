...
function Is-BackgroundPixel([System.Drawing.Color]$c) {
  ...
  return ($max - $min) -lt 46 -and $c.R -gt 166 -and $c.G -gt 166 -and $c.B -gt 166
}
...
function Save-NoBorderBubble($inputPath, $outputPath) {
  $src = [System.Drawing.Bitmap]::FromFile($inputPath)
  try {
    $bounds = Find-ContentBounds $src
    $size = [Math]::Ceiling([Math]::Max($bounds.Width, $bounds.Height) * 0.96)
    ...
    try {
      ...
      for ($y = 0; $y -lt $dstSize; $y++) {
        for ($x = 0; $x -lt $dstSize; $x++) {
          ...
          $b = $p.B
          $alpha = $p.A
          if ($dist -gt ($radius - $fade)) {
            $edge = ($radius - $dist) / $fade
            $alpha = [int]([Math]::Max(0, [Math]::Min(255, $alpha * $edge)))
            $r = [int]($r + (255 - $r) * 0.22)
            $g = [int]($g + (255 - $g) * 0.22)
            $b = [int]($b + (255 - $b) * 0.22)
          } elseif ($dist -gt ($radius - $fade - $soften)) {
            $mix = ($dist - ($radius - $fade - $soften)) / $soften
            $r = [int]($r + (255 - $r) * 0.1 * $mix)
            $g = [int]($g + (255 - $g) * 0.1 * $mix)
            $b = [int]($b + (255 - $b) * 0.1 * $mix)
          }
...
foreach ($item in $bubbleMap) {
  ...
  Save-NoBorderBubble $input (Join-Path $out $item.Out)
}

Write-Output "Reprocessed no-border bubbles into $out"
